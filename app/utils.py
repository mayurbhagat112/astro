...
def generate_astrological_summary(planet_positions):
    ...
    prompt = (
        "Based on the following planetary positions, please provide a detailed astrological description:\n"
        ...
        "\nPlease include interpretations for each planet and their significance in the context of astrology."
    )
    ...
    outputs = model.generate(
        inputs["input_ids"], 
        max_length=600,  # Increased length for more detailed summaries
        num_beams=8,  # Increased beam search for better quality
        no_repeat_ngram_size=2,
        early_stopping=True  # Stop early if the end of the text is reached
    )
